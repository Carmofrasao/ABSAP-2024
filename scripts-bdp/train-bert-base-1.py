...
def preprocess_review_final(row):
    row['texto']  = get_aspect_phrase(row['texto'], 0, min(len(row['texto']), 511))
    row['aspect'] = row['aspect'] if 'aspect' in row else get_aspect_phrase(row['texto'], 0, min(len(row['texto']), 63)) # row['texto'] = get_aspect_phrase(row['texto'], int(row['start_position']), int(row['end_position']))
    ### row['polarity'] = str(int(0) + 1) #str(int(row['polarity']) + 1) # row['polarity'] = str(int(row['polarity']) + 1)
    return row
...
tokenized_datasets_test = tokenized_datasets_test.remove_columns(['id', 'texto', 'aspect', 'start_position', 'end_position']) #tokenized_datasets_test = tokenized_datasets_test.remove_columns(['id', 'texto', 'aspect', 'start_position', 'end_position'])
...
tokenized_datasets_final = preprocessed_datasets_final.map(lambda x: tokenizer(x['texto']), batched=True) #.map(lambda x: tokenizer(x['texto'], truncation=True, padding='max_length', max_length=1024), batched=True) #(lambda x: tokenizer(x['texto']), batched=True)
# tokenized_datasets_final = tokenized_datasets_final.rename_column('polarity', 'target')
tokenized_datasets_final = tokenized_datasets_final.remove_columns(['texto', 'aspect']) # tokenized_datasets_final = tokenized_datasets_final.remove_columns(['texto', 'aspect', 'start_position', 'end_position'])
